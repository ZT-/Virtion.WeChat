...
namespace Virtion.Util
{
    ...
    {
        private static byte[] ucHexChars = Encoding.UTF8.GetBytes("0123456789ABCDEF");

        private static byte[] lcHexChars = Encoding.UTF8.GetBytes("0123456789abcdef");

        private static byte urlEscapeChar = 37;

        private static byte urlSpace = 43;

        private static byte[] urlForbidden = Encoding.UTF8.GetBytes("@&;:<>=?\"'/\\!#%+$,{}|^[]`");

        private static byte qpEscapeChar = 61;

        private static byte qpSpace = 95;

        private static byte[] qpForbidden = Encoding.UTF8.GetBytes("&;=?\"'%+_");
        ...
        {
            byte[] array = WWWTranscoder.Decode(Encoding.UTF8.GetBytes(toEncode), WWWTranscoder.urlEscapeChar, WWWTranscoder.urlSpace);
        ...
        {
            byte[] array = WWWTranscoder.Decode(Encoding.UTF8.GetBytes(toEncode), WWWTranscoder.qpEscapeChar, WWWTranscoder.qpSpace);
            return e.GetString(array, 0, array.Length);
        }
